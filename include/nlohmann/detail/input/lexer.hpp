...
// copying every scanned character eagerly. When supports_seek exists the first
// overload is selected (an exact match beats the ellipsis), otherwise SFINAE
// removes it and the second overload reports false.
template<typename InputAdapterType>
constexpr bool input_adapter_supports_seek(decltype(InputAdapterType::supports_seek)* /*unused*/)
{
    return InputAdapterType::supports_seek;
}

template<typename InputAdapterType>
constexpr bool input_adapter_supports_seek(...)
{
    return false;
}
...
class lexer : public lexer_base<BasicJsonType>
{
    ...
    static constexpr bool lazy_token_string = input_adapter_supports_seek<InputAdapterType>(nullptr);
...
scan_number_done:
    ...
    */
    char_int_type get()
    {
        ...
        if (!lazy_token_string && JSON_HEDLEY_LIKELY(current != char_traits<char_type>::eof()))
        {
            token_string.push_back(char_traits<char_type>::to_char_type(current));
        }
